...
class _RippleEffectState extends State<RippleEffect>
  ...
  Widget build(BuildContext context) {
    final baseColor = widget.color ?? 
        (widget.isFromPartner ? AppColors.secondary : AppColors.primary);
...
class _GlowTrailPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      final paint = Paint()
        ..color = Color.fromRGBO(color.red, color.green, color.blue, opacity * 0.6)
        ..style = PaintingStyle.fill
  ...
  Color _getParticleColor(math.Random random) {
    switch (widget.gestureType) {
      case GestureType.doubleTap:
        return [AppColors.loveRed, Colors.pink, Colors.pinkAccent][random.nextInt(3)];
      case GestureType.swipeUp:
        return [AppColors.highFiveGold, Colors.amber, Colors.orange][random.nextInt(3)];
      case GestureType.circleMotion:
        return [AppColors.calmBlue, Colors.lightBlue, Colors.cyan][random.nextInt(3)];
      case GestureType.pinch:
        return [AppColors.pinchOrange, Colors.deepOrange, Colors.amber][random.nextInt(3)];
    }
  }
...
class _ParticleBurstPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var particle in particles) {
      ...
      final y = center.dy + math.sin(particle.angle) * distance - (progress * 50);
  ...
  void _drawLargeHeart(Canvas canvas, Offset center, double size, double opacity) {
    final paint = Paint()
  ...
  void _drawHighFive(Canvas canvas, Offset center, double size, double opacity) {
    final paint = Paint()
      ..color = Color.fromRGBO(AppColors.highFiveGold.red, AppColors.highFiveGold.green, AppColors.highFiveGold.blue, opacity)
      ..style = PaintingStyle.fill;
  ...
  void _drawSparkle(Canvas canvas, Offset center, double size, double opacity) {
    final paint = Paint()
      ..color = Color.fromRGBO(AppColors.calmBlue.red, AppColors.calmBlue.green, AppColors.calmBlue.blue, opacity)
      ..style = PaintingStyle.fill;
  ...
  void _drawPinch(Canvas canvas, Offset center, double size, double opacity) {
    final paint = Paint()
      ..color = Color.fromRGBO(AppColors.pinchOrange.red, AppColors.pinchOrange.green, AppColors.pinchOrange.blue, opacity)
      ..style = PaintingStyle.fill;
    
    canvas.drawCircle(Offset(center.dx - size * 0.4, center.dy), size * 0.3, paint);
    canvas.drawCircle(Offset(center.dx + size * 0.4, center.dy), size * 0.3, paint);
  }
...
class _TouchPointIndicatorState extends State<TouchPointIndicator>
  ...
  Widget build(BuildContext context) {
    ...
    final baseColor = widget.isFromPartner ? AppColors.secondary : AppColors.primary;
