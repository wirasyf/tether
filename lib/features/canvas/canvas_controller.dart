...
class CanvasController extends ChangeNotifier {
  ...
  void _subscribeToPartnerEvents() {
    _touchSubscription = _socketService.incomingTouches.listen(
      _handlePartnerTouch,
    );
    _gestureSubscription = _socketService.incomingGestures.listen(
      _handlePartnerGesture,
    );
  }
  ...
  void _handleGesture(GestureEvent gesture) {
    ...
    switch (gesture.type) {
      ...
      case GestureType.hug:
        _hapticService.longPressStart(); // Warm sustained vibration
        break;
      case GestureType.kiss:
        _hapticService.doubleTapLove(); // Soft pop
        break;
      case GestureType.heartbeat:
        _hapticService.doubleTapLove(); // Pulsing heartbeat
        break;
      case GestureType.thinkingOfYou:
        _hapticService.swipeUpHighFive(); // Gentle pulse
        break;
      case GestureType.goodnight:
        _hapticService.circleCalm(); // Soft fading
        break;
    }
  ...
  /// Send a gesture manually from UI (e.g., gesture picker)
  void sendManualGesture(GestureEvent gesture) {
